...
#define BACKLIGHT PG2
#define ROT1_A PE4
#define ROT1_B PE5
#define MODE_BUTTON PF2
#define BAND_BUTTON PF1
#define VOLROT_A PE6
#define VOLROT_B PE7
#define VOL_BUTTON PF0
#define CLAR_POT PF3
...
#define lcd_on() PORTG |= (1 << BACKLIGHT)
#define lcd_off() PORTG &= ~(1 << BACKLIGHT)
...
volatile uint16_t bandf, tx, pll_n;
volatile bool rx_att;
...
uint8_t updateVolume(uint8_t vol) {
  ...
  addr = 0x07; // Address for audio conf
...
uint8_t updateFreq(double freq, int8_t clar, uint8_t all_update) {
  ...
  if (all_update) {
    ...
    pll_n = (uint16_t)lround(freq_lo/(double)100);  // 100 kHz step
    ref_freq = 170*freq_lo/pll_n;  // 170 ref divider value
...
uint8_t updateSettings (void) {
  ...
  switch (mode) {  
  case LSB:
    byte = 0b01111000; // Set USB (LO+)
    break;
  case USB:
    byte = 0b01110000; // Set LSB (LO+)
    break;
  case CWN:
    byte = 0b01100000; // Set narrow LSB (LO+)
    break;
  case AM:
    byte = 0b01000000;
    break;
  ...
  err = TWI_write_5bytes(fpga_addr, byte, 0x00, 0x00, tx_att << 7 , 0x00);
  if (err)
    return err;
...
int main(void)
{
  ...
  static const char string_intro_row2[] PROGMEM = "Daniels radio";

  DDRA = 0xFF;
  DDRB = 0xFF;
  DDRC = 0xFF;
  DDRD = 0xFF;
  DDRE = ~(uint8_t)((1 << ROT1_A)|(1 << ROT1_B)|(1 << VOLROT_A)|(1 << VOLROT_B));
  DDRF = ~(uint8_t)((1 << VOL_BUTTON)|(1 << MODE_BUTTON)|(1 << BAND_BUTTON)|(1 << CLAR_POT));
  DDRG = 0xFF;  
  ...
  PORTE = (uint8_t)((1 << ROT1_A)|(1 << ROT1_B)|(1 << VOLROT_A) | (1 << VOLROT_B));
  PORTF = (uint8_t)((1 << VOL_BUTTON)|(1 << MODE_BUTTON)|(1 << BAND_BUTTON));
  PORTG = 0x00;
  ...
  while (1) {
	...
	rssi_max = rssi;
      rssi_count ++;
      if ((!rx_att && rssi_count > 2) || (rx_att && rssi_count > 6)) {
	rssi_count = 0;
	if (rssi_max < 9)
	  rssi = 0;
	else 
	...
	else if (rx_att && rssi>6) {
	  sprintf(buffer,"S---9+++   ");
	  lcd_goto(0x40);
	  lcd_puts(buffer);
	}
	else if (rx_att && rssi < 7) {
	  rx_att = false;
	  err = updateRFFE();
	}
	else if (rssi < 2) { 
